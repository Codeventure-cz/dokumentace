...
Komunikace pak může probíhat oběma směry. Směr \uv{hra na backend} je použit pouze jednou, a to při načtení, když hra potřebuje vykreslit ostrovy. Této akce je docíleno pomocí připravené funkce v Reactu, která prostřednictvím mutace \uv{getUnityIslands} kontaktuje server, aby zaslal zpět nové ostrovy. Tato funkce lze použít pomocí Unity metody \uv{GetIslands}.\par
Druhý směr komunikace je častější a to \uv{backend na hru}. Pro tento směr je potřeba nejdříve navázat spojení pomocí subscription \uv{unityCommunication}. Backend má pak možnost provést jakoukoliv veřejnou metodu umístěnou v komunikačním skriptu. Jako argument může přiložit data, která chce předat. Tato data jsou ve formátu JSON a je třeba je po přijetí deserializovat. Používá se například pro přejetí kamery na jiný ostrov, nebo nastavení objektů do scény se zadáním.
